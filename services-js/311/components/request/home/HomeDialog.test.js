...
describe('choose page', () => {
  ...
  beforeEach(async () => {
    ...
    const ctx = makeServerContext('/request', { stage: 'choose', description: 'Thanos is attacking' });
    initialProps = await HomeDialog.getInitialProps(ctx);
  });
...
describe('integration', () => {
  ...
  test('description and suggestions', async () => {
    ...
    expect(serviceLink.first().parent().props().href).toEqual('/request?code=CSMCINC&description=Thanos%20is%20attacking');
  });
});
