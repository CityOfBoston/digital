...
import type { SearchAddressQuery, SearchAddressQueryVariables } from './graphql/types';
...
export default async function searchAddress(
  ...
  query: string): Promise<?SearchAddressPlace> {
  const args: SearchAddressQueryVariables = { query };

  const response: SearchAddressQuery = await loopbackGraphql(SearchAddressGraphql, args);
