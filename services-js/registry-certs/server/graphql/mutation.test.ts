...
import {
  ...
  MarriageIntentionCertificateOrderItemInput,
...
const DEFAULT_MARRIAGE_INTENTION_ITEM: MarriageIntentionCertificateOrderItemInput = {
  firstName: 'Danielle',
  lastName: 'Cage',
  alternateSpellings: '',
  birthDate: new Date('01 March 2006 00:00:00 GMT').toISOString(),
...
describe('Mutation resolvers', () => {
  ...
  describe('submitMarriageIntentionCertificateOrder', () => {
    it('throws if a validator fails', async () => {
      await expect(
        resolvers.Mutation.submitMarriageIntentionCertificateOrder(
          {},
          {
            ...DEFAULT_ORDER,
            item: DEFAULT_MARRIAGE_INTENTION_ITEM,
            contactName: '',
          },
          { registryDb } as any
        )
    ...
    it('sends a charge to Stripe', async () => {
      ...
      await resolvers.Mutation.submitMarriageIntentionCertificateOrder(
        {},
        {
          ...DEFAULT_ORDER,
          item: {
      ...
      expect(chargesCreate).toHaveBeenCalledWith({
        ...
        source: 'tok_test',
        capture: false,
        description: 'Marriage Intention certificates (Registry)',
        statement_descriptor: 'CITYBOSTON*REG + FEE',
        metadata: expect.objectContaining({
          'webapp.name': 'registry-certs',
          'webapp.nodeEnv': 'test',
