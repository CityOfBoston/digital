...
export default function YesNoUnsureComponent(props: Props): JSX.Element {
  function radioItem(
    value: YesNoUnknownAnswer,
    text: string
  ): React.ReactChild {
    return (
      <label
        className={`${RADIOITEM_SHARED_STYLING} ${YESNOITEM_STYLING} ${
          props.questionValue === value ? 'selected' : ''
        } ${
          props.questionValue && props.questionValue !== value ? 'inactive' : ''
        }`}
      >
        <svg
