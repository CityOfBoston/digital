...
interface Props {
  ...
  options: Options[];
  /**
   * Shown when `value` matches `placeholderValue`. Not listed as a choosable
   * option in the open menu (disabled + hidden).
   */
  placeholder?: string;
  /** Defaults to `-1`. */
...
export default function RegistryCCSelectDropDown(props: Props): JSX.Element {
  ...
  const selectedLabel = (() => {
    if (placeholder && (value === placeholderValue || value === undefined)) {
      return placeholder;
    }
  ...
  useLayoutEffect(() => {
    ...
    mirror.textContent = selectedLabel || '';
    // Space for caret icon + tight padding-right (kept in sync with CS_SELECT)
    const caretSpace = 12;
    select.style.width = `${Math.ceil(mirror.offsetWidth) + caretSpace}px`;
  }, [selectedLabel, value, options, placeholder]);
  ...
  return (
    ...
    >
      ...
      <div className="select--wrapper">
        ...
        >
          {placeholder && (
            <option value={placeholderValue} disabled hidden>
              {placeholder}
            </option>
          )}
          {options.map(option => (
        ...
        {/* Mirror used only to measure label width for shrink-wrapping */}
        <span ref={mirrorRef} css={MIRROR_STYLING} aria-hidden="true" />
      </div>
    </span>
  );
}

const MIRROR_STYLING = css`
  position: absolute;
  visibility: hidden;
  pointer-events: none;
  white-space: pre;
...
const CS_SELECT = css`
  ...
  .select--wrapper {
    ...
    select {
      ...
      padding-right: calc(0.15em + 0.3rem);
